...
class RRTConnect:
    ...
    def planning(self, link_info_main):
        for i in range(self.max_iter):
            ...
            if self.check_collision(new_node, link_info_main):
                new_node = self.avoid_obstacle(nearest_node, new_node, link_info_main)
            if new_node and not self.check_collision(new_node, link_info_main):
    ...
    def steer(self, from_node, to_node, extend_length=1.0):
    ...
    def avoid_obstacle(self, nearest_node, new_node, link_info_main, step_size=1.0):
        for _ in range(5):  # 尝试5次不同方向
            direction = np.random.uniform(-1, 1, size=6)
