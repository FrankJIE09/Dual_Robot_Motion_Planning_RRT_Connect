...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while i < seg + 1 and r_i < seg + 1:
        ...
        start = np.array(last_ik_joint_r[1:7])
        goal = np.array(ik_joint_r[1:7])
        # 计算 start 到 goal 的向量
        direction = goal - start
        # 计算这个向量的长度
        length = np.linalg.norm(direction)
        # 如果长度大于 1，调整 goal 为从 start 出发的单位向量
        if length > 0.005:
            # 单位化方向向量
            unit_vector = direction / length
            # 将 start 加上单位向量得到新的 goal
            new_goal = start + unit_vector * 0.005
        else:
            new_goal = goal
        # 如果长度不大于 1，则不修改 goal
