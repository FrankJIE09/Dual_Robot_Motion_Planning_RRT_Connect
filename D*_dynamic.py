...
class DStarLite:
    ...
    def update_vertex(self, node):
        ...
        if (self.calculate_key(node), node) in self.open_list:
...
# 模拟移动障碍物
def move_obstacles(obstacles, steps):
    new_obstacles = []
    for obs in obstacles:
        new_obstacles.append((obs[0] + steps, obs[1], obs[2]))
    return new_obstacles

# 更新障碍物
for steps in range(1, 4):
    new_obstacles = move_obstacles([(3, 3, 3), (3, 4, 3), (4, 3, 3)], steps)
    d_star_lite.update_obstacles(new_obstacles)
    new_path = d_star_lite.find_path(ax)
    print(f"Updated D* Lite Path at step {steps}:", new_path)
    plt.pause(1)  # 暂停1秒以观察障碍物和路径变化
